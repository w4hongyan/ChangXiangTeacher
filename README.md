...
- ✅ 倒计时显示：显示距离学期结束的天数
- ✅ 日历涂色：不同类型的事件以不同颜色显示
- ✅ 事件管理：支持添加各种类型的事件（普通事件、考试、假期、活动等）

#### 📋 文档模板中心（完成度：100%）
- ✅ 常用模板库：教案模板、课程计划、家长会通知
- ✅ 快速生成：班级通讯录、学生花名册、座位表、字帖、练习题、英语听写纸
...
- `schedules` - 课程表（班级、教师、科目、时间安排）
- `semesters` - 学期表（学期名称、开始结束日期）
- `calendar_events` - 日历事件表（事件标题、日期、类型）
- `document_templates` - 文档模板表（模板名称、分类、内容）
...
<div align="center">
  ...
  | 🪑 排位管理 | ✅ 已完成 | 95% | 可视化布局 + 智能分配 |
  | 📊 成绩管理 | 🚧 开发中 | 85% | 完整 CRUD + 导入导出 |
  | 🏆 积分管理 | 🚧 开发中 | 85% | 基础框架已完成 |
  | 📅 课程表管理 | ✅ 已完成 | 100% | 课程安排 + 上课提醒 |
  | 📆 学期日历 | ✅ 已完成 | 100% | 学期管理 + 倒计时提醒 |
  | 📋 文档中心 | ✅ 已完成 | 100% | 教学文档模板库 |
